  ...
  columns = [
    {
      ...
      render: (text, record) => (
        <Fragment>
          <Button.Group>
            <Button icon="edit" size="small" onClick={() => this.showEditModal(record)} />
      ...
      () => {
        if (sorter.field) {
          this.setState(
            ...
            () => {
              if (firstPage) this.handleSearchFirst();
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem label="电话" {...formLayout}>
            {getFieldDecorator('phoneNumber', {
              rules: [{ required: true, message: '请输入的话' }],
              initialValue: this.state.current.phoneNumber || '',
            })(<Input placeholder="请输入" />)}
          </FormItem>
          <FormItem
            label={
          ...
          <FormItem label="角色" {...formLayout}>
            {getFieldDecorator('roleIds', {
              ...
              initialValue: this.state.current.roleIds,
              valuePropName: 'value',
            })(
              <Select mode="multiple" loading={this.state.selectLoading} allowClear={true}>
          ...
          <FormItem label={<span>已启用</span>} {...formLayout}>
            {getFieldDecorator('isActive', {
              initialValue: this.state.current.isActive || false,
          ...
          >
            {getFieldDecorator('adminRemark', { initialValue: this.state.current.adminRemark })(
              <Input.TextArea style={{ minHeight: 32 }} placeholder="管理员备注" rows={2} />
            )}
          </FormItem>
        </Form>
      );
